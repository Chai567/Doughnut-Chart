...
class App extends React.Component {
  ...
  generateWords = () => {
    ...
    words.map((word,index) =>(
      dwords[index]=word.text,
      dcount[index]=word.value
    ));
       console.log(dwords);
       console.log(dcount);
    
    this.setState({ words });
    this.setState({dwords:dwords, dcount:dcount});
  };
  ...
  render() {
    const { settings, context, words ,dcount,dwords} = this.state;
    
    return (
      
      <div className="monday-app">
<Doughnut data={{labels: dwords,
  datasets: [{
    data:dcount,
